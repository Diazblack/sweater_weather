...
describe Giff do
  ...
  it 'exists' do
    expect(@Giff).to be_a(Giff)
  end

  it 'has attributes' do

    expect(@Giff.id).to eq("YnH5fDZPcNJfi")
    expect(@Giff.summary).to eq("Partly cloudy until afternoon.")
    expect(@Giff.time).to eq(1546585200)
    expect(@Giff.giff_url).to eq("https://giphy.com/gifs/weather-dancing-YnH5fDZPcNJfi")
  end
end
